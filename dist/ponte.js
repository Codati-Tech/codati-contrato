...
function injetarCss() {
    if (document.querySelector('style[data-codati-ponte-css]'))
        return;
...
export function iniciarPonte() {
    ...
    (function () {
        ...
        function onMensagem(tipo, fn) {
            if (!tratadoresDeMensagem[tipo])
                tratadoresDeMensagem[tipo] = [];
            tratadoresDeMensagem[tipo].push(fn);
        }
        function send(type, payload) {
            if (window.parent === window)
                return;
            window.parent.postMessage(Object.assign({ source: 'codati-preview', type: type }, payload || {}), PANEL_ORIGIN);
        }
        ...
        function markLocked() {
            all('[data-codati-path]').forEach(function (node) {
                if (isOpen(node.getAttribute('data-codati-path')))
                    node.removeAttribute('data-codati-locked');
                else
                    node.setAttribute('data-codati-locked', '');
            });
        }
        ...
        function inventory() {
            return {
                ...
                links: unique(all('[data-codati-link]').map(function (node) {
                    return node.getAttribute('data-codati-link');
                })).map(function (path) {
                    return { path: path };
                }),
                lists: unique(all('[data-codati-list]').map(function (node) {
        ...
        function syncEmptyLists() {
            ...
            all('[data-codati-list]').forEach(function (list) {
                ...
                if (!rule || !rule.canAdd || itemsOf(path).length > 0)
                    return;
                var holder = document.createElement('div');
                holder.setAttribute('data-codati-ui', '');
                holder.setAttribute('data-codati-ui-inline', '');
                holder.appendChild(button('+ Adicionar ' + rule.itemLabel, function () {
        ...
        function activate(node) {
            ...
            if (kindOf(node) === 'text') {
                ...
                if (node.children.length === 0 && node.textContent !== trimmed)
        ...
        function onClick(event) {
            if (!editing)
                return;
            if (event.target.closest('[data-codati-ui]'))
                return;
            ...
            items.forEach(function (item, index) {
                ...
                [item].concat(all('[data-codati-item], [data-codati-path], [data-codati-link]', item)).forEach(function (node) {
                    ['data-codati-item', 'data-codati-path', 'data-codati-link'].forEach(function (attribute) {
                        ...
                        if (value !== null)
                            node.setAttribute(attribute, replaceItemPath(value, before[index], after));
                    });
                });
        ...
        function describeNewPaths(listPath) {
            ...
            Object.keys(labels).forEach(function (path) {
                var shape = shapeIn(listPath, path);
                if (shape && !names[shape])
                    names[shape] = labels[path];
            });
            ...
            itemsOf(listPath).forEach(function (item) {
                all('[data-codati-path], [data-codati-link]', item).forEach(function (node) {
                    ...
                    if (!shape)
                        return;
                    if (added && open[shape] && added.indexOf(path) === -1)
                        added.push(path);
                    if (names[shape] && !labels[path])
                        labels[path] = names[shape];
                });
            });
            editable = added;
        }
        ...
        function applyListAction(path, action, index) {
            ...
            if (action === 'add') {
                ...
                if (active && item.contains(active))
                    deactivate();
                if (hovered && item.contains(hovered))
                    hovered = null;
        ...
        function onMessage(event) {
            if (event.origin !== PANEL_ORIGIN)
                return;
            var data = event.data;
            if (!data || data.source !== 'codati-panel')
                return;
            ...
            switch (data.type) {
                ...
                case 'patch':
                    if (typeof data.path === 'string')
                        applyPatch(data.path, data.value);
                    break;
                case 'focus':
                    if (typeof data.path === 'string')
                        focusPath(data.path);
        ...
        function buildUi() {
            ...
            ui.image.appendChild(button('Trocar foto', function () {
            ...
            ui.rich.appendChild(button('Editar texto', function () {
                var target = ui.rich.target;
                if (target)
                    send('edit-richtext', { path: target.getAttribute('data-codati-path') });
            }));
        ...
        function refresh() {
            ...
            if (link && !isOpen(link.getAttribute('data-codati-link')))
                link = null;
            if (link) {
