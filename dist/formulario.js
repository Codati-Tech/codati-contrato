import { separarContato } from "./leads.js";
/**
 ...
 */
export function formularioDaOrigem(manifesto, origem) {
    ...
    if (!formulario) {
        ...
        throw new Error(`O bloco de formulário aponta para "${id}", que o codati.manifest.json não declara em "forms". ` +
            (declarados.length > 0
 ...
 */
export function controlesDoFormulario(formulario) {
...
export const SEM_CONTATO = 'Deixe um WhatsApp com DDD ou um e-mail para a gente responder.';
/**
 ...
 */
export function montarEnvio(entradas, siteSlug, origem, pagina) {
    ...
    if (!raiz.email && !raiz.phone)
        return { acao: 'recusar', motivo: SEM_CONTATO };
    data._pagina = pagina;
    return {
        acao: 'enviar',
