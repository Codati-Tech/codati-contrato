...
const estrutural = new Set<string>(CHAVES_ESTRUTURAIS);
...
const Chave = z
  ...
  .min(1)
  .refine(
    (k) => !k.split('.').some((parte) => estrutural.has(parte)),
    (k) => ({
      message:
        `"${k}" é chave estrutural. Ela decide como o bloco se comporta, não o ` +
...
export const ListaSchema: z.ZodType<Lista> = z.lazy(() =>
  z
    .object({
      key: Chave,
    ...
    .refine(
      (l) => l.minItems === undefined || l.maxItems === undefined || l.minItems <= l.maxItems,
      { message: 'minItems maior que maxItems: nenhuma quantidade satisfaz a lista' },
    ),
);
...
export const SecaoSchema = z
  ...
  .refine((s) => s.fields.length > 0 || s.lists.length > 0, {
    message:
      'seção sem campo e sem lista não declara nada — o painel a mostraria vazia',
  });
 ...
 */
export const CHAVES_FIXAS_DO_LEAD = ['name', 'email', 'phone', 'message'] as const;
...
export const CampoDeFormularioSchema = z.object({
  key: z
    .string()
    .regex(
      CHAVE_DE_CAMPO_DE_FORMULARIO,
      'a chave do campo usa letra minúscula, número e _, começando por letra (até 40)',
    ),
 ...
 */
export const PaginasSchema = z.record(
  z.string().regex(/^[a-z]{2}(-[a-z]{2})?$/i, 'idioma no formato pt-br'),
  z
    .array(
      z
        .string()
...
export const ManifestoSchema = z.object({
  ...
  forms: z
    .record(
      z.string().regex(ID_DE_FORMULARIO, 'id de formulário em letra minúscula, número e hífen (até 40)'),
      FormularioSchema,
    )
    .optional(),
});
 ...
 */
export function carregarManifesto(bruto: unknown): Manifesto {
  const parsed = ManifestoSchema.safeParse(bruto);

  if (!parsed.success) {
    throw new Error(
      'codati.manifest.json inválido: ' +
        parsed.error.issues
 ...
 */
export function caminhosDeclarados(manifesto: Manifesto): string[] {
  const out: string[] = [];

  for (const [tipo, secao] of Object.entries(manifesto.sections)) {
    for (const campo of secao.fields) out.push(`${tipo}.${campo.key}`);
    for (const lista of secao.lists ?? []) achatarLista(out, `${tipo}.${lista.key}`, lista);
  }

  return out.sort();
}

function achatarLista(out: string[], prefixo: string, lista: Lista): void {
  for (const campo of lista.fields) out.push(`${prefixo}[].${campo.key}`);
